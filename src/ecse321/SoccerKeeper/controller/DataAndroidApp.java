...
/**
 * This class is used to read and write data for the android application to and from a file or an asset inside the application.
 * If the application is installed for the first time, it loads the original file, if it was installed, it loads a custom file from the user's phone.
 * @author Vivien
 *
 */
public class DataAndroidApp{
	...
	/**
	 * This class wipes off the phone's file, and writes all the data from the application to the file.
	 * @param myContext android parameter to access files
	 */
...
//            CSVWriter cErase = new CSVWriter(new FileWriter(dataFile, false), ',');
    ...
    /**
     * Uses either the readFromFileAsset() function if no file has been created or the readFromInternalFile() function if there is already an existant file.
     * @param myContext android parameter to access file
    ...
    /**
     * Reads the data from the application original file and turns it into seasons, leagues, teams and player
     * @param myContext android parameter to access file
     * @return season array with all the elements of the file
     */
...
//                        seasons.get(j).getLeagues().get(seasons.get(j).getLeagues().size()-1).getTeams().get(seasons.get(j).getLeagues().get(seasons.get(j).getLeagues().size()-1).getTeams().size()-1).addPlayer(tempPlayer);
    ...
    /**
     * Reads the data from the phone's internal file and turns it into seasons, leagues, teams and player
     * @param myContext android parameter to access file
     * @return season array with all the elements of the file
     */
...
//                        seasons.get(j).getLeagues().get(seasons.get(j).getLeagues().size()-1).getTeams().get(seasons.get(j).getLeagues().get(seasons.get(j).getLeagues().size()-1).getTeams().size()-1).addPlayer(tempPlayer);
    ...
    /**
     * Checks if the internal file has been created yet.
